...
public class RecurlyTest extends TestCase {
	...
	@Test
	public void testErrors() throws JAXBException { 
		JAXBContext context = JAXBContext.newInstance(Errors.class);
		
		String response = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
			+ "<errors>"
				+ "<error field=\"number\" code=\"invalid\">credit card number is not a valid credit card number</error>"
				+ "<error field=\"year\" code=\"in_past\">Year must be in the future</error>"
			+ "</errors>";
		
		Unmarshaller unmarshaller = context.createUnmarshaller();
		Object object = unmarshaller.unmarshal(new ByteArrayInputStream(response.getBytes()));
		
		Assert.assertTrue(object instanceof Errors);
		
		Errors errors = (Errors) object;
		Assert.assertEquals(2, errors.error.size());
	}
}
